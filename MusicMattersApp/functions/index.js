        ...
        switch (type) {
            case VALID_TYPES[0]: // artist_confirmation
                processEvent(data.eventID).then(processedData => {
                    return generateConfirmationOrInvoice(processedData.event, processedData.client, processedData.venue, 'Confirmation')
                }).then(handleSuccess).catch(handleError);
                break;

            case VALID_TYPES[1]: // invoice
                processEvent(data.eventID).then(processedData => {
                    return generateConfirmationOrInvoice(processedData.event, processedData.client, processedData.venue, 'Invoice')
                }).then(handleSuccess).catch(handleError);
                break;

            case VALID_TYPES[2]: // booking_list
                processEvents(data.venueID, data.month, data.year).then(processedData => {
                    return generateBookingOrCalendar(processedData.events, processedData.venue, processedData.month, processedData.year, 'Booking')
                }).then(handleSuccess).catch(handleError);
                break;

            case VALID_TYPES[3]: // calendar
                processEvents(data.venueID, data.month, data.year).then(processedData => {
                    return generateBookingOrCalendar(processedData.events, processedData.venue, processedData.month, processedData.year, 'Calendar')
                }).then(handleSuccess).catch(handleError);
                break;
        }
    });
...
const generateConfirmationOrInvoice = function(event, client, venue, type) {
    return new Promise((resolve, reject) => {
        ...
        return Promise.all([
            ...
            Drive.uploadArtistConfirmation(event, client, venue, pdf)
        ]).then(pdfs => {
            if(type == 'Confirmation')
 ...
 * year -> an intger representing the year
 */
const generateBookingOrCalendar = function(events, venue, month, year, type) {
    return new Promise((resolve, reject) => {
        if(type == 'Booking')
            pdf = PDF.generateBookingList(month, year, events, venue);
        else
            pdf = PDF.generateCalendar(month, year, events);
