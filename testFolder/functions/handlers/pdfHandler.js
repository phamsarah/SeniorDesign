...
exports.generateInvoice = function (event, client, venue) {
  ...
  content.push({
    text: [
      ...
      ' artist/perfomers agree to perform agree to perform live music at ' +
        venue.name +
        ', ' +
        eventAddress +
        ', ' +
      ...
      '. ' +
        ...
        ', ' +
        venue.address.state +
        ' agrees to pay the above named artists ',
      { text: priceString, style: 'underlined' },
      ', and said payment to be paid upon completion of this performance.',
    ],
    style: 'formText',
  });
  //
...
exports.generateArtistConfirmation = function (event, client, venue) {
  ...
  content.push({
    text: [
      '\u200B\t\t',
      { text: stringifyNames(client.performers), style: 'underlined' },
      ' agree(s) to perform live music at ' +
        venue.name +
        ', ' +
        eventAddress +
        ', ' +
      ...
      '. ' +
        ...
        ', ' +
        venue.address.state +
        ' agrees to pay the above named artist(s) ',
      { text: priceString, style: 'underlined' },
...
*/
  ...
  content.push({
    text: [
      ...
      'If he cannot be reached please call James Bohiem ',
      { text: '334-224-3814', style: 'underlined' },
...
exports.generateBookingList = function (month, year, events, venue) {
  ...
  bookingListTable.push(
    ['Artist', 'Gig Time', 'Compensation'].map((headerName) => {
      return {
        text: headerName,
        style: 'columnHeader',
      };
  ...
  while (currentDate.getMonth() === firstDate.getMonth()) {
    ...
    matchingEvents.forEach((event, i) => {
      ...
      let time = [Util.toAMPM(event.start), Util.toAMPM(event.end)].join(
        ' to '
      );
